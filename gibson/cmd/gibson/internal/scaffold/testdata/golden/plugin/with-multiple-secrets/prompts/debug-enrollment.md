...
4. The CLI runs `capabilitygrant.Bootstrap → Discover → Register` and
   persists `~/.gibson/plugin/byte-identity-multi.host_key` plus
   `~/.gibson/plugin/byte-identity-multi.runtime.json` (both mode 0600).
...
Should list `byte-identity-multi.host_key` and `byte-identity-multi.runtime.json` (both mode
