...
4. The CLI runs `capabilitygrant.Bootstrap → Discover → Register` and
   ...
   `~/.gibson/plugin/byte-identity-secret.runtime.json` (both mode 0600).
...
Should list `byte-identity-secret.host_key` and `byte-identity-secret.runtime.json` (both mode
