# Deploy checklist for byte-identity
...
spec:
  ...
  template:
    spec:
      containers:
      - name: plugin
        image: <registry>/byte-identity:0.1.0
        env:
        - name: GIBSON_URL
          value: <daemon URL>
        ...
        - name: GIBSON_BOOTSTRAP_TOKEN
          valueFrom:
            secretKeyRef: { name: byte-identity-bootstrap, key: token }
        volumeMounts:
        - name: host-key
          mountPath: /home/nonroot/.gibson/plugin/byte-identity
        ports:
...
- The plugin must be allowed to write `~/.gibson/plugin/byte-identity/`
